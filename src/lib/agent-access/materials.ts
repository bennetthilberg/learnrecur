...
// Material-library ingestion lives in its own mutation module so the read
// tools below stay small and safe to audit. Re-exporting the handlers here
// keeps the existing materials MCP entrypoint stable for registration code.
export {
  AgentMaterialIngestionError,
...
export async function getAgentMaterialOutline(
  auth: AgentAuthContext,
  rawInput: unknown,
) {
  ...
  if (!material?.activeRevisionId)
    throw new AgentMaterialError(
      "material_not_found",
      "The material was not found.",
    );
  if (
    input.expected_revision_id &&
    input.expected_revision_id !== material.activeRevisionId
  ) {
    throw new AgentMaterialError(
      "stale_material_revision",
      "The active material revision changed.",
    );
  }
  const revision = await prisma.materialRevision.findFirst({
    where: {
      id: material.activeRevisionId,
      userId: auth.userId,
...
export async function searchAgentMaterialExcerpts(
  auth: AgentAuthContext,
  rawInput: unknown,
) {
  ...
  if (!material)
    throw new AgentMaterialError(
      "stale_material_revision",
      "The material or revision was not found.",
    );
  ...
  if (input.section_ids?.length) {
    ...
    if (ownedSections !== input.section_ids.length) {
      throw new AgentMaterialError(
        "material_not_found",
        "One or more material sections were not found.",
      );
    }
  }
