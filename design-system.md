...
Exercise queue state strips should also be ready-first. The ready count is the actionable inventory signal; running/full/locked labels are supporting state and should not receive equal layout weight. Keep these strips flat and inline rather than boxed into equal stat cells, and drop dividers in wrapped responsive layouts.
