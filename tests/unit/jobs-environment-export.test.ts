...
it("exports a staging project's production deployment only to its staging namespace", async () => {
  ...
  const values = Object.fromEntries(["DATABASE_URL", "S3_BUCKET_NAME", "CLERK_SECRET_KEY", "NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY", "NEXT_PUBLIC_APP_URL", "RESEND_API_KEY", "RESEND_FROM_EMAIL", "GEMINI_API_KEY", "WORKOS_API_KEY", "MCP_RESOURCE_URL"].map((key) => [key, `fixture-${key}`]));
...
it("writes only worker settings into a new immutable encrypted revision", async () => {
  const send = vi.fn().mockResolvedValue({});
  const values = Object.fromEntries(["DATABASE_URL", "S3_BUCKET_NAME", "CLERK_SECRET_KEY", "NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY", "NEXT_PUBLIC_APP_URL", "RESEND_API_KEY", "RESEND_FROM_EMAIL", "GEMINI_API_KEY", "WORKOS_API_KEY", "MCP_RESOURCE_URL"].map((key) => [key, `fixture-${key}`]));
  await expect(exportWorkerConfiguration({ ...context, ...values, AWS_SECRET_ACCESS_KEY: "do-not-copy" }, send, async () => {})).resolves.toEqual({ exported: true, revision: "verified-revision", count: 10 });
  expect(send).toHaveBeenCalledTimes(11);
