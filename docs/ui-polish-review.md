...
Replaced native popup selects with Mantine Select in practice preferences, text comparison, reminder scheduling, custom-session collections, and material-import collections. The existing searchable practice timezone picker shares the same theme. Menus use the existing white surface, border, ink, blue tint, and a Phosphor caret, with 44px options and no drop shadow. Reminder timezone selection is searchable. Named form controls preserve their serialized values through Mantine's hidden inputs, and selections cannot be accidentally deselected.

Design review addressed five mismatches: browser-blue selection became the existing blue tint; the heavy popup shadow was removed; square popup corners now match app menus; cramped native options became comfortable touch targets; inconsistent native arrow glyphs became one Phosphor caret. Inspected open menus at 390px and 1280px. Keyboard arrows, Enter, Escape, focus retention, saved practice preferences, reminder hour form values, collection selection, inherited preferences, and normal/custom practice were covered by browser checks. The initial keyboard test used an unsupported End shortcut; it now verifies the supported arrow-key flow. No scheduling, grading, or database logic changed.

Final verification: lint, TypeScript, 1,099 unit tests, Prisma validation/generation, and production build passed. Four retention/custom-session browser scenarios passed, followed by both corrected select scenarios (plus setup/cleanup). Generated formatting-only changes were discarded. No push or PR.
