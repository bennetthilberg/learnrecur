...
Muse the text, headings, IDs, and locators for every stored chunk in the
structurally selected scope. Muse scores each chunk; the server requires one
...
This scan covers stored text chunks in the selected scope. It cannot recover
text that ingestion and page OCR did not extract, and a complete scan does not
